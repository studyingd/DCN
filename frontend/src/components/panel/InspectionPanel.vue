    ...
    <div :class="embedded ? '' : 'inline-panel-body'">
      <el-tabs v-model="activeTab">
        ...
        <el-tab-pane label="即时巡检" name="instant">
          <div class="section">
            <div class="section-header">
              ...
              <span v-if="checkedDevices.length > 0" class="selected-count"> 已选 {{ checkedDevices.length }} 台 </span>
            </div>
          ...
          <div class="section">
            ...
            <div v-if="inspMode === 'custom'" v-loading="catalogLoading" class="custom-items">
              <el-checkbox-group v-model="customItems">
                <el-checkbox v-for="item in itemCatalog" :key="item.type" :value="item.type" :label="item.type">
                  {{ item.label }}
          ...
          <div class="section run-bar">
            ...
            <span v-if="checkedDevices.length > 0" class="run-info">
              将巡检 {{ checkedDevices.length }} 台设备， 每台约 {{ inspMode === 'quick' ? '5' : '13' }} 项
            </span>
          </div>
          ...
          <div v-if="runResults.length > 0" class="section">
            <div class="section-header">
              ...
              <span class="result-summary">
                共 {{ runResults.length }} 台， 成功 {{ runSucceeded }} 台， 异常
                {{ runResults.length - runSucceeded }} 台
              </span>
            </div>
            <InspectionResultCard v-for="(r, i) in runResults" :key="r.record_id ?? r.device_id ?? i" :result="r" />
          </div>
        </el-tab-pane>
        ...
        <el-tab-pane label="巡检历史" name="history">
          ...
          <el-table v-loading="historyLoading" :data="historyItems" stripe size="small">
        ...
        <el-tab-pane label="巡检报告" name="report">
          ...
          <div v-if="report" v-loading="reportLoading">
            ...
            <div class="report-summary">
              ...
              <div class="report-card">
                <div class="report-card-value" style="color: var(--el-color-warning)">
                  {{ report.overall_health.warning_pct }}%
                </div>
                <div class="report-card-label">警告率</div>
              </div>
              <div class="report-card">
                <div class="report-card-value" style="color: var(--el-color-danger)">
                  {{ report.overall_health.critical_pct }}%
                </div>
            ...
            <!-- By target type -->
            <div v-if="Object.keys(report.by_target_type).length > 0" class="section">
              <div class="section-header"><span class="section-title">按设备类型分布</span></div>
              <el-descriptions :column="3" border size="small">
                <el-descriptions-item
                  v-for="(info, type) in report.by_target_type"
                  :key="type"
                  :label="targetTypeLabel[type] || type"
                >
            ...
            <!-- Top warnings -->
            <div v-if="report.top_warnings.length > 0" class="section">
  ...
  <el-dialog v-model="detailVisible" title="巡检详情" width="750px" destroy-on-close>
    <div v-if="detailData" v-loading="detailLoading">
      <div class="detail-header">
        ...
        <el-tag :type="recordStatusTag[detailData.status] || 'info'" size="small">{{
          recordStatusLabel[detailData.status] || detailData.status
        }}</el-tag>
        ...
        <span class="detail-duration">{{
          detailData.duration_ms ? (detailData.duration_ms / 1000).toFixed(1) + 's' : ''
        }}</span>
      </div>
      <div class="detail-counts">
        <el-tag type="success" effect="plain" size="small">正常 {{ detailData.normal_count }}</el-tag>
        <el-tag v-if="detailData.warning_count" type="warning" effect="plain" size="small"
          >警告 {{ detailData.warning_count }}</el-tag
        >
        <el-tag v-if="detailData.critical_count" type="danger" effect="plain" size="small"
          >严重 {{ detailData.critical_count }}</el-tag
        >
        <el-tag v-if="detailData.error_count" type="info" effect="plain" size="small"
          >错误 {{ detailData.error_count }}</el-tag
        >
      </div>
...
// ── Labels ──
const targetTypeLabel: Record<string, string> = { linux: 'Linux', windows: 'Windows' }
const targetTypeTag: Record<string, string> = { linux: 'success', windows: 'primary' }
// core 是自动化运维「健康巡检」固定使用的模式，不补这一项历史表会直接显示英文 core
const modeLabels: Record<string, string> = { standard: '标准', quick: '快速', custom: '自定义', core: '核心' }
...
const recordStatusTag: Record<string, string> = {
  completed: 'success',
  partial: 'warning',
  failed: 'danger',
  running: 'info',
...
async function loadCatalog() {
  ...
  // 以首台设备的操作系统决定巡检项目录（target_type 只剩 linux / windows）
  const firstDevice = checkedDevices.value[0]
  const targetType = (firstDevice.os_system || '').toLowerCase().includes('windows') ? 'windows' : 'linux'
...
async function loadHistory() {
  ...
  try {
    // 用接口自己的参数类型，塞进后端不认的筛选条件会直接编译报错
    const params: NonNullable<Parameters<typeof inspectionAPI.listRecords>[0]> = {
